...
%....... Compute new acceleration from equation of motion using Newton
         ...
         while ((err > tol) && (n < itmax))
                    n = n + 1;
                    xnew = bn + eta*anew;
                    r = k*xnew*(1 + b*xnew^2)^(-1/2);
                    gp = m + (k*xnew*(-1/2*((1 + b*xnew^2)^(-3/2))...
                        *2*b*xnew)+ k*(1 + b*xnew^2)^(-1/2))*eta; 
                    g  = m*anew + r - F;
                    anew = anew - g/gp;
                    err = norm(g);
          end 
...
%.... Plot displacement vs. time 
      ...
      xlabel('t'); ylabel('Disp.'); title({'Displacement vs. Time';'(Softener)'});
      p = plot(history(1:iout,1),history(1:iout,2),'-r');

%.... Plot force vs. displacement    
      fig=figure(2); clf; grid on; axis square; hold on;
      xlabel('Disp.'); ylabel('Force'); title({'Force vs. Displacement';'(Softener)'});
