...
%---- Forcing function parameters ---------------------------------------*      
      resonance = 1;              % =0 for no resonance
      Fo1 = 0.0;                 % Constant forcing function
      Fo2 = 1.0;                  % Amplitude of forcing function
      Fwo = 0.33;                 % Frequency of forcing function
...
%---- Initial displacement and velocity multipliers ---------------------*
      Xo = 35.0;                   % Initial displacement scale
...
%.... Set which figures to do (1=yes, 0=no)      
      ...
      dofig9 = 1;                 % plot force vs. displacement  
      
%.... Numerical analysis parameters
      dt     = 0.01;              % analysis time step
      beta   = 0.7;               % Numark Method parameter
...
%     resonance in the mode shapes.
      ...
      if model == 1    
        for i = 1:nbodies
          force(i) = (Fo1 + Fo2*sin(Fwo*t))*((i)+(i-1))*ampm;
        end
      else
          force = Fo1 + Fo2*sin(Fwo*t)*ones(nbodies,1);
      end
...
%     ground motion
      ...
      aold = (M)\(force - (R + C*vo)); % Linear case
      
%     Compute the initial acceleration for integrating the earthquake
%     ground motion
      aold1 = zeros(nbodies,1);
      Eq = zeros(nbodies,1);
      Eqaold = Eq;
