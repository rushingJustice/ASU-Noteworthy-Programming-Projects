...
%---- INPUT PROBLEM DATA ------------------------------------------------*
      nbodies = 10;                % number of masses in system
%---- Start and end times for anaysis -----------------------------------*      
      to = 0.0;                   % Initial time (usu. zero)
      tf = 500.0;                 % Final time
      t = to;                      
      plotmax = 1.0;             % max and min x value for plot range
%---- Mass, stiffness, damping, length multipliers ----------------------*
      Mo = 50.0;                  % Mass scaling value
      Ko = 65.0;                  % Stiffness scaling value
      Xi = 3;                  % Damping ratio scaling value
      So = 0.4;                    % Yield scaling value
...
%---- Forcing function parameters ---------------------------------------*      
      ...
      Fo1 = 0.1;                  % Constant forcing function
      Fo2 = 0.1;                  % Amplitude of forcing function
...
%---- Fake Earthquake motion --------------------------------------------*
      ...
      Eq  = 0;
      tD  = tf;                   % Duration of earthquake
      earthquake = 1;             % =1 create earthquake force
%------------------------------------------------------------------------*
...
%.... Set which figures to do (1=yes, 0=no)      
      ...
      dofig6 = 1;                 % plot ground acceleration  
      dofig7 = 0;                 % slectively plot mode shapes  
      dofig8 = 1;                 % plot ground displacement  
